...
request* new_request() {
  ...
  request_ptr->method = malloc(MAX_SIZE);
  request_ptr->target = malloc(MAX_SIZE);
  return request_ptr;
}

response* new_response() {
  response* response_ptr = calloc(1, sizeof(response));
  response_ptr->body = malloc(MAX_SIZE);
  return response_ptr;
}
...
void send_response(int client_socket, response* response_ptr) {
  char buffer[2048];
  sprintf(buffer, "%s %hd %s\r\n\r\n%s", HTTP_VERSION, response_ptr->status, "OK", response_ptr->body);
...
void handle_request(int client_socket) {
  ...
  sprintf(response_ptr->body, "Request was Method:%s Target:%s", request_ptr->method, request_ptr->target);
